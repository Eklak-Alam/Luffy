...
import { ArrowRight, Github, ChevronDown } from "lucide-react"; // Added ChevronDown
...
export default function DevOpsProjects() {
  ...
  // STATE: Track the exact number of projects to show on mobile (starts at 3)
  const [visibleCount, setVisibleCount] = useState(3);

  useGSAP(() => {
    // Desktop-only GSAP Stacking Animation
    let mm = gsap.matchMedia();

    mm.add("(min-width: 768px)", () => {
      const wrappers = gsap.utils.toArray(".devops-sticky-wrapper");

      wrappers.forEach((wrapper, index) => {
        if (index === wrappers.length - 1) return;

        const innerCard = wrapper.querySelector(".devops-card-inner");
  ...
  return (
    <section 
      id="infrastructure" 
      ref={sectionRef} 
      className="relative w-full bg-background py-16 md:py-24 overflow-visible"
    >
      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-6 lg:px-8">
        ...
        <div className="relative flex flex-col pb-4 md:pb-[10vh]">
          {allDevOpsProjects.map((project, index) => {
            ...
            // Hide the card on mobile if its index is greater than or equal to visibleCount
            ...
            return (
              <div 
                ...
                className={`devops-sticky-wrapper relative desktop-sticky w-full mb-8 md:mb-20 ${mobileDisplayClass}`}
                style={{ "--sticky-top": stickyTop }}
              >
                <div className={`devops-card-inner w-full border rounded-[1.5rem] md:rounded-[2rem] p-6 sm:p-8 md:p-10 flex flex-col ${layoutClass} gap-6 md:gap-12 origin-top ${themeClass}`}>
                  
                  {/* === MINIMALIST NUMBER SIDE (Hidden on Mobile) === */}
                  ...
                  <div className="w-full lg:w-[55%] flex flex-col justify-center py-2 relative z-10">
                    ...
                    <div className="flex flex-wrap items-center gap-4 md:gap-6 mt-auto">
                      ...
                      {project.links?.github && (
                        ...
                        >
                          <Github className="w-4 h-4 md:w-5 md:h-5 group-hover:scale-110 group-hover:-rotate-12 transition-transform duration-300" />
